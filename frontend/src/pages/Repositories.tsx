...
import { Search, Star, GitFork, ExternalLink, Filter, SortAsc, RefreshCw } from 'lucide-react';
...
export const Repositories: React.FC = () => {
  ...
  const languages = useMemo(() => {
    const langCounts = repositories.reduce((acc, repo) => {
      if (repo.language) {
  ...
  // Get language count for display
  const getLanguageCount = (lang: string) => {
    if (lang === 'all') return repositories.length;
    return repositories.filter(repo => repo.language === lang).length;
  };
              ...
              <SelectContent>
                {languages.map(lang => (
                  <SelectItem key={lang} value={lang}>
                    {lang === 'all' 
                      ? `All Languages (${getLanguageCount(lang)})` 
      ...
      {filteredRepositories.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filteredRepositories.map((repo, index) => (
            ...
            >
              <Card className="h-full hover:shadow-card transition-smooth group gradient-card backdrop-blur-sm border-border/50 cursor-pointer" onClick={() => navigate(`/repositories/${repo.name}`)}>
                <CardHeader>
