...
import { Search, GitPullRequest, ExternalLink, Filter, Clock, User, RefreshCw } from 'lucide-react';
...
export const PullRequests: React.FC = () => {
  ...
  const { pullRequests, isLoading } = usePullRequests(statusFilter === 'all' ? 'open' : statusFilter);
  const { repositories } = useRepositories();

  // Get repository names with PR counts
  const repositoryNames = useMemo(() => {
    const repoCounts = repositories.reduce((acc, repo) => {
      const count = pullRequests.filter(pr => pr.repo_name === repo.name).length;
      if (count > 0) {
        acc[repo.name] = count;
      }
      return acc;
  ...
  const filteredPullRequests = useMemo(() => {
    return pullRequests.filter(pr => {
      ...
      const matchesRepository = repositoryFilter === 'all' || pr.repo_name === repositoryFilter;
      const matchesStatus = statusFilter === 'all' || pr.state === statusFilter;
      return matchesSearch && matchesRepository && matchesStatus;
    });
  }, [pullRequests, searchQuery, repositoryFilter, statusFilter]);
              ...
              <SelectContent>
                {repositoryNames.map(repo => (
                  <SelectItem key={repo} value={repo}>
                    {repo === 'all' 
                      ? `All Repositories (${statusCounts.all})` 
                      : `${repo} (${pullRequests.filter(pr => pr.repo_name === repo).length})`
                    }
                  </SelectItem>
                ))}
              </SelectContent>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0">
                      <Avatar className="h-10 w-10">
                        <AvatarImage src={pr.author_avatar_url} alt={pr.author_username} />
                        <AvatarFallback>
